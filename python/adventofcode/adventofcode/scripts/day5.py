...
# rearrange them in a series of carefully-planned steps. After the crates are rearranged,
...
# Elves know where they should stand to be ready to unload the final supplies. After the
...
class SupplyStacks:
    def __init__(self):
        _data = open(os.getcwd() + "/../../problem_specs/day5.txt", "r")
        stacks_and_instructions = _data.read().splitlines()
        _data.close()
        dividing_line = stacks_and_instructions.index("")
        self.stacks = stacks_and_instructions[0:dividing_line]
        self.instructions = stacks_and_instructions[dividing_line + 1 : :]

    def _parse_stacks(self) -> Dict[int, List[str]]:
        """Get the crates in each stack as a dictionary.

        Returns
        -------
        Dict[int, List[str]]
            A dictionary of the stacks (numbered) and an ordered List
            of the crates in the stack. First element is on top of the stack.
        """
        stack_indices = [
            (idx, eval(element))
            for idx, element in enumerate(self.stacks[-1])
            if not element.isspace()
        ]

        crate_dict = {}
        for stack_loc, stack_idx in stack_indices:
            crates_in_stack = []
            for line in self.stacks[0:-1]:
                try:
                    if not line[stack_loc].isspace():
                        crates_in_stack.append(line[stack_loc])
                except IndexError:
                    continue
            crate_dict[stack_idx] = crates_in_stack

        return crate_dict

    def _parse_instruction(self, instruction: str) -> Tuple[int, int, int]:
        """Given a statement of instructions, return the important numerical
        values in the form of (move, from, to).

        Parameters
        ----------
        instruction : str
            An instruction from the input file.

        Returns
        -------
        Tuple[int, int, int]
            Values representing the (move, from, to) values in the instruction.
        """
        return tuple(
            eval(entry) for entry in instruction.split(sep=" ") if entry.isnumeric()
        )

    def CrateMover9000(self) -> Dict[int, List[str]]:
        """Follow the instructions under the CrateMover9000 assumption.

        Returns
        -------
        Dict[int, List[str]]
            An updated crate_dict
        """
        crate_dict = self._parse_stacks()
        for instruction in self.instructions:
            _move, _from, _to = self._parse_instruction(instruction=instruction)
            for idx in range(_move):
                crate = crate_dict[_from].pop(0)
                crate_dict[_to].insert(0, crate)
        return crate_dict

    def CrateMover9001(self) -> Dict[int, List[str]]:
        """Follow the instructions under the CrateMover9001 assumption.
        ...
        for instruction in self.instructions:
            _move, _from, _to = self._parse_instruction(instruction=instruction)
            crate_slice = crate_dict[_from][0:_move]
            crate_dict[_to] = crate_slice + crate_dict[_to]
            for crate in crate_slice:
                crate_dict[_from].remove(crate)
        return crate_dict

    def part1(self):
        final_dict = self.CrateMover9000()
        top_crates = ""
    ...
    def part2(self):
        final_dict = self.CrateMover9001()
        top_crates = ""
        for _, value in final_dict.items():
            top_crates += value[0]
        return top_crates
