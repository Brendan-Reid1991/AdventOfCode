...
# rope. If the head moves far enough away from the tail, the tail is pulled toward the
# head.
...
# However, be careful: more types of motion are possible than before, so you might want
...
class Snake:
    def __init__(self, knots: int = 2):
        _data = open(os.getcwd() + "/../../problem_specs/day9.txt", "r")
        self.instructions = [x.split(" ") for x in _data.read().splitlines()]
        _data.close()
        self.num_knots = knots
        self.knots = {idx: Position(0, 0) for idx in range(self.num_knots)}
        self.head = Position(0, 0)
        self.tail = Position(0, 0)

        self.tail_history = [self.tail]

    def _up(self, places: int):
        return Position(0, places)

    def _down(self, places: int):
        return Position(0, -places)

    def _left(self, places: int):
        return Position(-places, 0)

    def _right(self, places: int):
        return Position(places, 0)

    def _move(self, command: str, places: int):
        if command == "U":
            return self._up(places)
        if command == "D":
            return self._down(places)
        if command == "L":
            return self._left(places)
        if command == "R":
            return self._right(places)

    def _move_knot(self, x_delta, y_delta):
        lateral_movement = "R" if x_delta > 0 else "L" if x_delta < 0 else None
        longitudinal_movement = "U" if y_delta > 0 else "D" if y_delta < 0 else None

        if lateral_movement is None:
            return self._move(longitudinal_movement, abs(y_delta) - 1)
        if longitudinal_movement is None:
            return self._move(lateral_movement, abs(x_delta) - 1)

        move_x = max(1, abs(x_delta) - 1)
        move_y = max(1, abs(y_delta) - 1)
        # print(lateral_movement, move_x)
        # print(longitudinal_movement, move_y)

        return self._move(lateral_movement, move_x) + self._move(
            longitudinal_movement, move_y
        )

    def move_snake(self, command: str, places: int):
        for _ in range(places):
            self.knots[0] += self._move(command, 1)

            for knot, position in self.knots.items():
                if knot == 0:
                    continue
                dx, dy = self.knots[knot - 1] - position
                if max(abs(dx), abs(dy)) > 1:
                    new_position = position + self._move_knot(dx, dy)
                    self.knots[knot] = new_position
                    if knot == self.num_knots - 1:
                        self.tail_history.append(new_position)
            # print(self.knots)

    def slither(self):
        for command, places in self.instructions:
            self.move_snake(command, eval(places))
        return len(set(self.tail_history))


if __name__ == "__main__":
    snake1 = Snake(knots=2)
    print(snake1.slither())

    snake2 = Snake(knots=10)
    print(snake2.slither())
