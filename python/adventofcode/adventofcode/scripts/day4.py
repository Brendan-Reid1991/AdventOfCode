...
# so several Elves have been assigned the job of cleaning up sections of the camp.
...
class CampCleanup:
    def __init__(self):
        _data = open(os.getcwd() + "/../../problem_specs/day4.txt", "r")
        self.pairings = _data.read().splitlines()
        _data.close()

    def sections(self, pair: str) -> Tuple[Set[int], Set[int]]:
        """Get the sections assigned to each elf in a given pair.

        Parameters
        ----------
        pair : str
            Input from the problem spec

        Returns
        -------
        Tuple[Set[int], Set[int]]
            All sections the elves have been assigned to in their range.
        """
        elf1, elf2 = pair.split(",")
        start1, stop1 = tuple(map(eval, elf1.split("-")))
        start2, stop2 = tuple(map(eval, elf2.split("-")))

        return set(range(start1, stop1 + 1)), set(range(start2, stop2 + 1))

    def part1(self):
        """Solve part1 of the puzzle."""
        fully_contained = 0
        for pair in self.pairings:
            elf1_sections, elf2_sections = self.sections(pair=pair)
            if elf1_sections <= elf2_sections or elf2_sections <= elf1_sections:
                fully_contained += 1
        return fully_contained

    def part2(self):
        """Solve part2 of the puzzle."""
        overlaps = 0
        for pair in self.pairings:
            elf1_sections, elf2_sections = self.sections(pair=pair)
            if elf1_sections & elf2_sections:
                overlaps += 1
        return overlaps
