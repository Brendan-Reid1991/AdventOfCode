...
# The Elves yell something about meeting back up with them upriver, but the river is
# too loud to tell exactly what they're saying.
...
# to be some kind of cathode-ray tube screen and simple CPU that are both driven by a
# precise clock circuit. The clock circuit ticks at a constant rate; each tick is called
# a cycle.

# Start by figuring out the signal being sent by the CPU. The CPU has a single register,
# X, which starts with the value 1. It supports only two instructions:
...
#  first cycle, X is 1. After the first cycle, the noop instruction finishes execution,
# doing nothing.
...
# value of register X is the starting value, 1, plus all of the other addx values up
# to that point: 1 + 15 - 11 + 6 - 3 + 5 - 1 - 8 + 13 + 4 = 21.)
...
# "vertical position": if the sprite's horizontal position puts its pixels
...
# top row of pixels left-to-right, then the row below that, and so on.
# The left-most pixel in each row is in position 0, and the right-most pixel in
...
class SignalProcessing:
    def __init__(self):
        _data = open(os.getcwd() + "/../../problem_specs/day10test.txt", "r")
        self.signals = _data.read().splitlines()
        _data.close()
        self.register = 1
        self.cycles = 1
        self.width = 40
        self.height = 6
        self.CRT = np.array([["." * self.width]] * self.height)

    def _parse_signal(self, signal: str):
        if signal == "noop":
            return "noop"
        return eval(signal.split(" ")[1])

    def _terminate_program_(self, terminate: int):
        return True if self.cycles == terminate else False

    def _convert(self, number):
        row = number // 41
        column = max(number % 41 - 1, 0)
        return row, column

    def draw(self):
        sprite_row, sprite_col = self._convert(self.register)
        pointer_row, pointer_col = self._convert(self.cycles)
        if sprite_row == pointer_row and abs(sprite_col - pointer_col) <= 1:
            draw_pixel = list(self.CRT[pointer_row][0])
            draw_pixel[pointer_col - 1] = "#"
            # print("".join(draw_pixel))
            self.CRT[pointer_row - 1] = ["".join(draw_pixel)]

    def iterate_to_cycle(self, terminate: int):
        self.register = 1
        self.cycles = 1
        self.draw()
        for signal in self.signals:
            instr = self._parse_signal(signal)
            if instr == "noop":
                self.cycles += 1
                self.draw()
                if self._terminate_program_(terminate):
                    break
    ...
    def signal_strength(self, terminate: int = None):
        self.iterate_to_cycle(terminate)
        return self.register * self.cycles

    def part1(self):
        return sum(
            [
                self.signal_strength(terminate=cycle)
...
if __name__ == "__main__":
    crt = SignalProcessing()
    crt.signal_strength()
    for num in range(1, 81):
        print(num, crt._convert(num))
